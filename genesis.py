...
def create_input_script(psz_timestamp):
    ...
    script_prefix = b'04ffff001d0104' + psz_prefix.encode() + bytes([len(psz_timestamp)])
    return script_prefix + psz_timestamp.encode()

def create_output_script(pubkey):
    script_len = b'\x41'
    OP_CHECKSIG = b'\xac'
    return script_len + bytes.fromhex(pubkey) + OP_CHECKSIG

def create_transaction(input_script, output_script, options):
    # обновляем создание транзакции с правильной длиной
    ...
    tx.num_outputs       = 1
    tx.out_value         = struct.pack('<q', options.value)  # 5000000000
...
def print_block_info(options, hash_merkle_root):
    print("block header: ")
    print("    version: " + str(1))
    print("    previous block: " + "0" * 64)
    print("    hash_merkle_root: " + hash_merkle_root.hex())
    print("    time: " + str(options.time))
    print("    bits: " + str(hex(options.bits)))

def announce_found_genesis(genesis_hash, nonce):
    print("\nGenesis Hash Found: " + genesis_hash.hex())
